...
public class KalahBoard {
	...
	/**
	 * evaluates the Board on who is more likely to win
	 *
...
/*
	...
	// Mit Alpha-Beta Erweiterung
	private int MinValue(KalahBoard state, int limit, int alpha, int beta) {
		KalahBoard copyBoard = new KalahBoard(state);
		if (copyBoard.finish()) {
			return copyBoard.getCurPlayer() == APlayer ? -1 : 1; // 1 = Player A wins / -1 = Player B wins
		}
	...
	// Mit Alpha-Beta Erweiterung
	private int MaxValue(KalahBoard state, int limit, int alpha, int beta) {
		KalahBoard copyBoard = new KalahBoard(state);
		if (copyBoard.finish())
			return copyBoard.getCurPlayer() == APlayer ? -1 : 1; // 1 = Player A wins / -1 = Player B wins
		if (limit <= 0)
